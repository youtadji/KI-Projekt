...
def benchmark_score(fen, player, board):
    start_time = time.time()
    for _ in range(1000):  # Repeat the scoring function 1000 times
        score_value = game.score(board, player)
    end_time = time.time()
...
for case in benchmark_cases:
    ...
    visualized_board = main.visualize_board(main.reformulate(fen))
    avg_time = benchmark_score(fen, player, visualized_board)
    print(f"Benchmark for {description}: {avg_time:.6f} seconds per call")
    print(f"Benchmark for {description}: {(avg_time) * 1000:.4f} milliseconds for 1000 repetitions")
